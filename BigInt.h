...
public:
	...
	bool deduct(BigInt& numOne, BigInt& numTwo);
	...
	//Перегрузка оператора cравнения равенства ==
	//args:
	//BigInt& obj - сравниваемый объек
	//return - true - если одинаковы, false - если не одинаковы
	bool operator==(const BigInt& obj);

	//bool BigInt::operator< (BigInt& obj)
	//Перегрузка оператора cравнения <
	//args:
	//BigInt& obj - сравниваемый объект
	//return - true - если первый меньше второго, false - если первый больше второго
	...
	friend ofstream& operator<< (ofstream& os, BigInt& obj);

	friend ifstream& operator>> (ifstream& is, BigInt& obj);

	void writeInBin(fstream& binFile);

	void readFromBin(fstream& binFile);
