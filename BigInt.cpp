...
bool BigInt::deduct(BigInt& numOne, BigInt& numTwo) {
	bool pos = true;
...
const char* BigInt::divide(BigInt& numOne, BigInt& numTwo) {
	try {
		if (numTwo.getLongNumberValue(0) == 0)
			throw std::exception("Error! Division by zero is not possible.");
	}
	...
	try {
		...
		for (int i = 0; i < currentLength; i++) {
			if ((ULONG_MAX - this->longNumber[i]) / 10 < number) {
				throw std::overflow_error("Error! Overflow unsigned long.");
			}
...
ofstream& operator<< (ofstream& os, BigInt& obj) {
	os << obj.length;
	os << " ";
	for (int i = 0; i < obj.length; i++) {
		os << obj.longNumber[i];
...
ifstream& operator>> (ifstream& is, BigInt& obj) {
	is >> obj.length;
	obj.setArray();
	for (int i = 0; i < obj.length; i++) {
		is >> obj.longNumber[i];
...
void BigInt::writeInBin(fstream& os) {
	char tempLength = this->length + '0';
	os.put(tempLength);
	char* tempNumber = new char[this->length];
	for (int i = 0; i < this->length; i++) {
		tempNumber[i] = '0' + this->longNumber[i];
		os.put(tempNumber[i]);
	}
	os << endl;
}

void BigInt::readFromBin(fstream& os) {
	char tempLength;
	os.get(tempLength);
	...
	for (int i = 0; i < this->length; i++) {
		os.get(tempNumber[i]);
		this->longNumber[i] = tempNumber[i] - '0';
	}
}